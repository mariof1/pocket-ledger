...
from calendar_math import BILL_FREQUENCIES, month_span, month_value, monthly_bill_cents
from account_routes import register as register_account_routes
...
CREATE INDEX IF NOT EXISTS idx_statement_imports_profile ON statement_imports(profile_id, id DESC);
CREATE TABLE IF NOT EXISTS statement_import_items (
  batch_id INTEGER NOT NULL REFERENCES statement_imports(id) ON DELETE CASCADE,
  profile_id INTEGER NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  file_hash TEXT NOT NULL, source_row INTEGER NOT NULL,
  transaction_id INTEGER NOT NULL UNIQUE REFERENCES transactions(id) ON DELETE CASCADE,
  UNIQUE(profile_id, file_hash, source_row)
);
...
def transfer_too_large(_error):
    ...
    return fail("The CSV statement must be 1 MB or smaller.", 413)
...
def protect_api():
    ...
    if request.endpoint in ("inspect_statement", "preview_statement", "save_statement"):
        request.max_content_length = 2_200_000
...
register_planning_routes(app, globals())
register_account_routes(app, globals())
register_statement_routes(app, globals())
